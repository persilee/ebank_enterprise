...
#import "SEFaceError.h"

NS_ASSUME_NONNULL_BEGIN

@interface SEFaceVerifyResult : NSObject
...
/**se
 ...
 isSuccess == NO  时, error中存储的具体错误信息,参考 SEFaceError.h
 */
@property (nonatomic, strong, readonly)SEFaceError* error;

+(instancetype)resultWith:(BOOL)isSuccess error:(SEFaceError *)error;
+(instancetype)resultWith:(BOOL)isSuccess error:(SEFaceError *)error sign:(NSString *)sign;
+(instancetype)resultWith:(BOOL)isSuccess error:(SEFaceError *)error sign:(NSString *)sign liveRate:(NSString *)liveRate similarity:(NSString *)similarity;
+(instancetype)resultWith:(BOOL)isSuccess error:(SEFaceError *)error sign:(NSString *)sign liveRate:(NSString *)liveRate similarity:(NSString *)similarity userImageString:(NSString *)userImageString;
