...
#include "CloudroomQueue.h"
#include "CloudroomVideoSDK_Def.h"
#import "CloudroomCommonType.h"
#import "CloudroomCommonType.h"
...
@property (nonatomic, assign) BOOL privEC; // 是否启用私有回声消除
@property (nonatomic, assign) BOOL privAgc; // 是否启用私有声音增益
@property (nonatomic, assign) BOOL HILevelEC; // 超强回声消息
...
@property (nonatomic, assign) VIDEO_WHRATE_TYPE whRate; // 视频比例（支持：WHRATE_16_9、WHRATE_4_3、WHRATE_1_1）
...
-(void)sendCustomMeetingMsgRslt:(NSString *)taskID sdkErr:(CRVIDEOSDK_ERR_DEF)sdkErr cookie:(NSString *)cookie;
...
-(void)stopScreenMark;
//请求屏幕共享
-(void)requestShare:(NSString *)shareId param:(NSString *)param;

-(void)cancelShareRequestion:(NSString *)shareId;

-(void)rejectShareRequestion:(NSString *)requestId param:(NSString *)param;
//赋予控制权限
...
-(void) setCustomizeScreenImg:(NSData*)yuvDat datLenght:(int)datLenght width:(int)width height:(int)height;
